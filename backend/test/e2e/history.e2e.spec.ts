...
// T22: histórico de atividades (extra Fase 6) — eventos no CRUD + leituras.
// Isolamento mongo e2e: MONGO_TEST_URL ?? MONGO_URL (banco de teste separado).
// NÃO parallel-safe: DELETE no setup + suíte sequencial (singleFork).
// Truncate MySQL: tasks ANTES de users (FK tasks.userId → users.id).
...
// Delay mínimo entre writes p/ evitar colisão de ms no occurredAt
// (o repositório também desempatava por _id, defesa em duas camadas).
...
describe('task history (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    app = await buildApp({ db, jwtSecret: TEST_JWT_SECRET, activityRepository: new MongoActivityRepository(mongo) })
  })
  ...
  it('criar→editar→concluir = [completed, updated, created] desc', async () => {
    ...
    const created = await app.inject({ method: 'POST', url: '/api/tasks', headers, payload: { title: 'com-histórico' } })
    ...
    const history = await app.inject({ method: 'GET', url: `/api/tasks/${task.id}/history`, headers })
    ...
    expect(events.map((e) => e.action)).toEqual(['completed', 'updated', 'created'])
    // DTO serializável.
    ...
    // Feed global espelha os mesmos 3 eventos em desc.
    const feed = await app.inject({ method: 'GET', url: '/api/activity', headers })
    expect(feed.statusCode).toBe(200)
    expect((feed.json() as Array<{ action: string }>).map((e) => e.action)).toEqual([
      'completed',
      'updated',
      'created',
    ])
  ...
  it('401 sem token nas 2 rotas', async () => {
    const history = await app.inject({ method: 'GET', url: '/api/tasks/1/history' })
  ...
  it('cross-user: history 404 + /activity vazio', async () => {
    ...
    const history = await app.inject({ method: 'GET', url: `/api/tasks/${id}/history`, headers: headersB })
    expect(history.statusCode).toBe(404)
    expect(history.json()).toMatchObject({ code: 'TASK_NOT_FOUND' })

    const feed = await app.inject({ method: 'GET', url: '/api/activity', headers: headersB })
    expect(feed.statusCode).toBe(200)
    expect(feed.json()).toEqual([])

    // Dono segue vendo o próprio evento.
    const own = await app.inject({ method: 'GET', url: `/api/tasks/${id}/history`, headers: headersA })
    expect(own.statusCode).toBe(200)
    expect((own.json() as Array<{ action: string }>).map((e) => e.action)).toEqual(['created'])
  })
  ...
  it('degradação: activity lança → CRUD segue (HIST-04)', async () => {
    ...
    const degradedApp = await buildApp({ db, jwtSecret: TEST_JWT_SECRET, activityRepository: throwing })
    try {
      ...
      const listed = await degradedApp.inject({ method: 'GET', url: '/api/tasks', headers })
      expect(listed.statusCode).toBe(200)
      expect((listed.json() as Array<{ title: string }>).map((t) => t.title)).toEqual(['sem-mongo v2'])

      const deleted = await degradedApp.inject({ method: 'DELETE', url: `/api/tasks/${id}`, headers })
