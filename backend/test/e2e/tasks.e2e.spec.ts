...
// T18: CRUD escopado por usuário — todo inject leva Bearer (helper register+login).
// Truncate: tasks ANTES de users (FK tasks.userId → users.id).
...
async function authHeaders(app: FastifyInstance): Promise<Record<string, string>> {
  userSeq += 1
  const email = `user${Date.now()}_${userSeq}@techx.com`
  ...
  const logged = await app.inject({ method: 'POST', url: '/api/auth/login', payload: { email, password } })
...
describe('tasks CRUD (e2e happy paths)', () => {
  ...
  it('fluxo completo: criar → listar → buscar por id → editar → toggle → excluir', async () => {
    ...
    const task = created.json() as { id: number; title: string; completed: boolean; userId: number }
    expect(task.id).toBeGreaterThan(0)
    expect(task.title).toBe('Aprender Fastify')
    expect(task.completed).toBe(false)

    // listar
    const listed = await app.inject({ method: 'GET', url: '/api/tasks', headers })
    expect(listed.statusCode).toBe(200)
    expect(listed.json()).toHaveLength(1)

    // buscar por id (M1)
    const got = await app.inject({ method: 'GET', url: `/api/tasks/${task.id}`, headers })
    ...
    const deleted = await app.inject({ method: 'DELETE', url: `/api/tasks/${task.id}`, headers })
    expect(deleted.statusCode).toBe(204)
    const gone = await app.inject({ method: 'GET', url: `/api/tasks/${task.id}`, headers })
  ...
  it('GET por id inexistente → 404 TASK_NOT_FOUND', async () => {
    const headers = await authHeaders(app)
    const res = await app.inject({ method: 'GET', url: '/api/tasks/999999', headers })
  ...
  it('lista em ordem crescente de position', async () => {
    ...
    for (const title of ['primeira', 'segunda', 'terceira']) {
      const res = await app.inject({ method: 'POST', url: '/api/tasks', headers, payload: { title } })
...
describe('tasks validation and error edge cases (e2e)', () => {
  ...
  it('POST com title de 256 chars → 400 com details', async () => {
    const headers = await authHeaders(app)
    const res = await app.inject({ method: 'POST', url: '/api/tasks', headers, payload: { title: 'x'.repeat(256) } })
    expect(res.statusCode).toBe(400)
  ...
  it('PATCH de id inexistente → 404 TASK_NOT_FOUND', async () => {
    const headers = await authHeaders(app)
    const res = await app.inject({ method: 'PATCH', url: '/api/tasks/888888', headers, payload: { title: 'x' } })
    expect(res.statusCode).toBe(404)
    expect(res.json()).toMatchObject({ code: 'TASK_NOT_FOUND' })
  })
  ...
  it('DELETE de id inexistente → 404 TASK_NOT_FOUND', async () => {
    const headers = await authHeaders(app)
    const res = await app.inject({ method: 'DELETE', url: '/api/tasks/888888', headers })
    expect(res.statusCode).toBe(404)
    expect(res.json()).toMatchObject({ code: 'TASK_NOT_FOUND' })
  })
  ...
  it('PATCH com body vazio → 400', async () => {
    const headers = await authHeaders(app)
    const created = await app.inject({ method: 'POST', url: '/api/tasks', headers, payload: { title: 'x' } })
    const id = (created.json() as { id: number }).id
    const res = await app.inject({ method: 'PATCH', url: `/api/tasks/${id}`, headers, payload: {} })
    expect(res.statusCode).toBe(400)
    expect((res.json() as { code: string }).code).toBe('VALIDATION_ERROR')
  })
  ...
  it('tipos errados → 400 (title numérico, completed texto, id não-numérico)', async () => {
    const headers = await authHeaders(app)
    const badTitle = await app.inject({ method: 'POST', url: '/api/tasks', headers, payload: { title: 123 } })
  ...
  it('erro inesperado → 500 padronizado INTERNAL_ERROR (simulado)', async () => {
    ...
    const boomApp = await buildApp({ db, jwtSecret: TEST_JWT_SECRET, taskService: boomService })
    try {
      const res = await boomApp.inject({ method: 'GET', url: '/api/tasks', headers })
...
describe('tasks reorder (e2e)', () => {
  ...
  async function createTitles(headers: Record<string, string>, titles: string[]): Promise<number[]> {
    const ids: number[] = []
  ...
  it('edges 400: vazio/duplicado/<1/não-inteiro → VALIDATION_ERROR {code,message,details}', async () => {
    ...
    for (const payload of cases) {
      const res = await app.inject({ method: 'PATCH', url: '/api/tasks/reorder', headers, payload })
      expect(res.statusCode).toBe(400)
      const body = res.json() as { code: string; message: string; details: unknown }
      expect(body.code).toBe('VALIDATION_ERROR')
      expect(typeof body.message).toBe('string')
...
// T18: proteção de rotas + escopo por usuário (401/404/A-B).
describe('tasks auth scope (e2e)', () => {
  ...
  it('401 sem token em todos os verbos (incl. reorder)', async () => {
    const noAuth: Array<{ method: 'GET' | 'POST' | 'PATCH' | 'DELETE'; url: string; payload?: unknown }> = [
    ...
    for (const req of noAuth) {
      const res = await app.inject({ method: req.method, url: req.url, ...(req.payload ? { payload: req.payload } : {}) })
  ...
  it('401 com token inválido em todos os verbos (incl. reorder)', async () => {
    ...
    const cases: Array<{ method: 'GET' | 'POST' | 'PATCH' | 'DELETE'; url: string; payload?: unknown }> = [
      { method: 'GET', url: '/api/tasks' },
      { method: 'GET', url: '/api/tasks/1' },
      { method: 'POST', url: '/api/tasks', payload: { title: 'x' } },
      { method: 'PATCH', url: '/api/tasks/reorder', payload: { ids: [1] } },
  ...
  it('create vincula userId do token; list é escopado (A não vê B)', async () => {
    ...
    const created = await app.inject({ method: 'POST', url: '/api/tasks', headers: headersA, payload: { title: 'de-A' } })
    expect(created.statusCode).toBe(201)
    expect((created.json() as { userId: number }).userId).toBeGreaterThan(0)

    const listA = await app.inject({ method: 'GET', url: '/api/tasks', headers: headersA })
    expect((listA.json() as Array<{ title: string }>).map((t) => t.title)).toEqual(['de-A'])
    const listB = await app.inject({ method: 'GET', url: '/api/tasks', headers: headersB })
  ...
  it('cross-user GET/PATCH/DELETE → 404 TASK_NOT_FOUND', async () => {
    const headersA = await authHeaders(app)
    const headersB = await authHeaders(app)
    const created = await app.inject({ method: 'POST', url: '/api/tasks', headers: headersA, payload: { title: 'secreta' } })
    const id = (created.json() as { id: number }).id
    ...
    // Dono ainda acessa.
    const got = await app.inject({ method: 'GET', url: `/api/tasks/${id}`, headers: headersA })
    expect(got.statusCode).toBe(200)
  })
  ...
  it('reorder cross-user → 404 TASK_NOT_FOUND com missingIds', async () => {
    const headersA = await authHeaders(app)
    const headersB = await authHeaders(app)
    const created = await app.inject({ method: 'POST', url: '/api/tasks', headers: headersA, payload: { title: 'a' } })
    const id = (created.json() as { id: number }).id
    const res = await app.inject({
      method: 'PATCH',
      url: '/api/tasks/reorder',
    ...
    expect(res.statusCode).toBe(404)
    const body = res.json() as { code: string; details: { missingIds: number[] } }
    expect(body.code).toBe('TASK_NOT_FOUND')
